...
class_y = 'AD'
Slice = 'Coronal'
data_type_train = 'GM_0.1-0.5'
data_type_val = 'GM_0.1-0.5'
num_fold = '5-fold'
class_fold = 'fold_5'
class_val = 2
...
def get_sequential_set(path, class_name, num_fold, fold_name, split_num):
    ...
    csvpath = f"f/csv_download_from_ADNI/{class_name}/{num_fold}/ADNI-1.csv"
    datainf = pd.read_csv(csvpath, usecols=['Image Data ID'])
    ...
    for index in index_list:
        ...
        for img in file_list:
            ...
            if img_name == id:
                file.append(path + img)

    sequential_file = []
    batch_size = num_per_subject
    for i in range(0, len(file) - 1, batch_size):

        batch = file[i: i + batch_size]
        batch = sorted(batch, key=lambda x: int(x.split('_')[-1].split('.')[0]))
        sequential_file.extend(batch)
...
class EnsembleNet(nn.Module):
    def __init__(self, backbone, backbone_weights, last_layer, num_classes):
        ...
        # init.constant_(self.soft_voting_1.weight, 0.2)
        # init.constant_(self.soft_voting_2.weight, 0.2)
    ...
    def forward(self, x):
        ...
        if self.last_layer == 'fc':
            features = []
            for imgs in x:
                # Gets all the feature values that need to be entered into the fully connected layer
                ...
                feature = torch.cat(feature, dim=1) # Connect feature values from different networks
        ...
        if self.last_layer == 'majority_voting':
            ...
            for imgs in x:
                ...
                y_i = torch.where(y_voting >= 3, torch.tensor([0, 1], device='cuda:0'),
...
def main():
    ...
    for trial in range(num_trials):
        ...
        model = EnsembleNet(backbone=models.resnet18,
                            backbone_grad=False,
                            backbone_weights=[f'best_model_{class_x}{class_y}_{Slice}_GM_0.5_5p_res18.pth',
                                              f'best_model_{class_x}{class_y}_{Slice}_GM_0.4_5p_res18.pth',
                                              f'best_model_{class_x}{class_y}_{Slice}_GM_0.3_5p_res18.pth',
                                              f'best_model_{class_x}{class_y}_{Slice}_GM_0.2_5p_res18.pth',
                                              f'best_model_{class_x}{class_y}_{Slice}_GM_0.1_5p_res18.pth'],
                            last_layer='conv',
                            num_classes=output).to(device)
        ## If you want to test with a test set, you just need to load the training weights here
        # model.load_state_dict(torch.load(f'best_ensemble_model_CNMCI_Oblique_Coronal_CE_5GM_wv.pth'))
        ...
        for epoch in range(num_epochs):
            ...
            if val_accuracy > best_accuracy:
                ...
                best_epoch = epoch + 1
                best_trial = trial + 1
                torch.save(model.state_dict(), f'best_ensemble_model_{class_x}{class_y}_{Slice}_CE_5GM_conv.pth')
